...
app.use(cors({
  origin: function (origin, callback) {
    ...
    // السماح بكل الأصول في بيئة التطوير أو إذا كان الطلب من الجوال
    if (!isRender) return callback(null, true);
    // في الإنتاج، نسمح فقط للعناوين المعروفة
    ...
    if (allowedOrigins.includes(origin) || origin === true) {
      ...
      callback(null, true); // للتجربة، نسمح مؤقتاً. في الإنتاج يمكنك تغييره إلى false.
    }
  },
  credentials: true,
  methods: ['GET', 'POST', 'PUT', 'DELETE', 'PATCH', 'OPTIONS'],
