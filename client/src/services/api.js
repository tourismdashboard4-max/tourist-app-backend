// client/src/services/api.js - after fixing createConversation and URL paths
// ✅ إضافة خريطة عكسية (old_id -> UUID) ودوال تحويل للاستخدام في فتح المحادثات
...
// ✅ خريطة عكسية: old_id -> UUID
let guidesOldIdToUuidMap = null;

// دالة مساعدة لجلب قائمة المرشدين وبناء الخريطتين
    ...
    try {
      ...
      guidesList.forEach(guide => {
        ...
        if (uuid && numericId && !isNaN(Number(numericId))) {
          ...
          reverseMap[Number(numericId)] = uuid;
        }
      });
      guidesIdMap = map;
      guidesOldIdToUuidMap = reverseMap;
      console.log('✅ Guides map loaded:', guidesIdMap);
      console.log('✅ Reverse guides map loaded:', guidesOldIdToUuidMap);
      return guidesIdMap;
    } catch (err) {
...
// ✅ دالة لتحويل old_id (رقمي) إلى UUID
export const convertOldIdToUuid = async (oldId) => {
  if (!guidesOldIdToUuidMap) {
    await loadGuidesMap();
  }
  const numericOldId = Number(oldId);
  if (isNaN(numericOldId)) return null;
  return guidesOldIdToUuidMap?.[numericOldId] || null;
};

// ✅ دالة لتحويل UUID إلى old_id (رقمي)
export const convertUuidToOldId = async (uuid) => {
  if (!guidesIdMap) {
    await loadGuidesMap();
  }
  return guidesIdMap?.[uuid] || null;
};
...
export const api = {
  ...
  saveProgramsToLocal(programs) { /* (نفس الكود السابق) */ },
  getProgramsFromLocal() { /* (نفس الكود السابق) */ },
  ...
  async sendUserNotification(userId, title, message, type = 'info') {
    try {
      return await this.sendNotification({ userId, title, message, type });
    } catch (error) {
  ...
  // ============================================
  // 💬 CHAT SERVICES (مع إصلاح createConversation)
  // ============================================
  ...
  async getUserConversations() {
    try {
      ...
      const data = await response.json();
      if (!response.ok) return { success: false, conversations: [], error: data.message };
      return {
        success: true,
  ...
  // ✅ createConversation المعدلة (تدعم UUID وتحول إلى old_id، وتضيف userId)
  async createConversation(participantId, type = 'direct', bookingId = null) {
    try {
      ...
      if (bookingId !== null && bookingId !== undefined) {
        ...
        if (!isNaN(numericBookingId)) validBookingId = numericBookingId;
        else console.warn('⚠️ bookingId is not a valid integer, ignoring:', bookingId);
      }
  ...
  async getConversationMessages(conversationId, page = 1, limit = 50) {
    try {
      const token = localStorage.getItem('token');
      if (!token) throw new Error('No authentication token found');
      const response = await fetch(`${API_BASE_URL}/api/chats/${conversationId}/messages?page=${page}&limit=${limit}`, {
        headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' }
      });
      const data = await response.json();
  ...
  // ============================================
  // 🎫 SUPPORT TICKETS (مع دوال Local Storage المحفوظة)
  // ============================================
  ...
  async updateTicketStatus(ticketId, status) {
    try {
      ...
      const data = await response.json();
      if (!response.ok) throw new Error(data.message || 'فشل تحديث حالة التذكرة');
      return data;
    } catch (error) {
  ...
  async createSupportChat(userId, userName, message) {
    try {
      ...
      if (ticketResponse.success) {
        await this.sendSupportMessage(ticketResponse.ticket.id, message);
